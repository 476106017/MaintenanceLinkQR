...
<template>
  <PageLayout navTitle="cs_feedback"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CsFeedbackData';
defineOptions({
  name: 'CsFeedbackList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/caresys/csFeedback/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/caresys/csFeedback/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
