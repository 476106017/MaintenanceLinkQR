...
<template>
  <PageLayout navTitle="cs_care_record"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CsCareRecordData';
defineOptions({
  name: 'CsCareRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/caresys/csCareRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/caresys/csCareRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
