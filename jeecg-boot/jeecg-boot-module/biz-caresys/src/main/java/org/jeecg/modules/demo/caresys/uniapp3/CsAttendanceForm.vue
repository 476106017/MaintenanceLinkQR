...
navigationBarTitleText: 'cs_attendance',
},
}
</route>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['companyId']"
                  :label="get4Label('所属公司')"
                  name='companyId'
                  prop='companyId'
                  placeholder="请选择所属公司"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['officeId']"
                  :label="get4Label('所属事务所')"
                  name='officeId'
                  prop='officeId'
                  placeholder="请选择所属事务所"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['employeeId']"
                  :label="get4Label('员工ID')"
                  name='employeeId'
                  prop='employeeId'
                  placeholder="请选择员工ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('工作日期')"
                  labelWidth="100px"
                  type="date"
                  name='workDate'
                  v-model:value="myFormData['workDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <DateTime
                  :label="get4Label('打卡上班时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='clockIn'
                  v-model="myFormData['clockIn']"
              ></DateTime>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <DateTime
                  :label="get4Label('打卡下班时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='clockOut'
                  v-model="myFormData['clockOut']"
              ></DateTime>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['late']"
                   :label="get4Label('是否迟到')"
                   name='late'
                   prop='late'
                   placeholder="请选择是否迟到"
                   inputMode="numeric"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['earlyLeave']"
                   :label="get4Label('是否早退')"
                   name='earlyLeave'
                   prop='earlyLeave'
                   placeholder="请选择是否早退"
                   inputMode="numeric"
...
const dataId = ref('')
const backRouteName = ref('CsAttendanceList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csAttendance/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csAttendance/edit':'/caresys/csAttendance/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
