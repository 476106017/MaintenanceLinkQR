...
layout: 'default',
style: {
navigationBarTitleText: 'cs_employee',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="cs_employee"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CsEmployeeData';
defineOptions({
  name: 'CsEmployeeList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/caresys/csEmployee/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/caresys/csEmployee/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
