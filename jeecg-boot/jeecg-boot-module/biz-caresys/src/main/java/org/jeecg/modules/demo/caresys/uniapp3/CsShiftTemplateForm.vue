...
navigationBarTitleText: 'cs_shift_template',
},
}
</route>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['templateName']"
                  :label="get4Label('班次模板名称')"
                  name='templateName'
                  prop='templateName'
                  placeholder="请选择班次模板名称"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('工作日设定（JSON）')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['workingDays']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('休息日设定（JSON）')"
                 labelWidth="100px"
                 type="textarea"
                 name='restDays'
                 prop='restDays'
...
const dataId = ref('')
const backRouteName = ref('CsShiftTemplateList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csShiftTemplate/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csShiftTemplate/edit':'/caresys/csShiftTemplate/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
