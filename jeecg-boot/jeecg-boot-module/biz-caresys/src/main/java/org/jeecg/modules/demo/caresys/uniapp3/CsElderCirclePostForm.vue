...
navigationBarTitleText: 'cs_elder_circle_post',
},
}
</route>
        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('发布者（老人）')"
                  name='residentId'
                  prop='residentId'
                  placeholder="请选择发布者（老人）"
                  :rules="[
                                  { required: true, message: '请输入发布者（老人）!'},
                  ]"
                  clearable
              />
        </view>
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('图片列表(JSON)')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['imageUrls']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <DateTime
                  :label="get4Label('发布时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='postTime'
                  v-model="myFormData['postTime']"
              ></DateTime>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('CsElderCirclePostList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csElderCirclePost/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csElderCirclePost/edit':'/caresys/csElderCirclePost/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
