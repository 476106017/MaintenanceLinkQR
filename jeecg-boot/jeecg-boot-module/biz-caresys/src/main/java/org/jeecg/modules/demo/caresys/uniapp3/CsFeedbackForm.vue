...
navigationBarTitleText: 'cs_feedback',
},
}
</route>
        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('来源（家属/员工）')"
                  name='source'
                  prop='source'
                  placeholder="请选择来源（家属/员工）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('关联老人或员工')"
                  name='targetId'
                  prop='targetId'
                  placeholder="请选择关联老人或员工"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <DateTime
                  :label="get4Label('反馈时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='feedbackTime'
                  v-model="myFormData['feedbackTime']"
              ></DateTime>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('CsFeedbackList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csFeedback/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csFeedback/edit':'/caresys/csFeedback/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
