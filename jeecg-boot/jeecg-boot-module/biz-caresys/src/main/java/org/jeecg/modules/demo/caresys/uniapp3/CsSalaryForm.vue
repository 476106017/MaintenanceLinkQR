...
navigationBarTitleText: 'cs_salary',
},
}
</route>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['salaryMonth']"
                  :label="get4Label('工资月份')"
                  name='salaryMonth'
                  prop='salaryMonth'
                  placeholder="请选择工资月份"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['workingHours']"
                   :label="get4Label('工时数')"
                   name='workingHours'
                   prop='workingHours'
                   placeholder="请选择工时数"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['baseSalary']"
                   :label="get4Label('基础工资')"
                   name='baseSalary'
                   prop='baseSalary'
                   placeholder="请选择基础工资"
                   inputMode="numeric"
...
const dataId = ref('')
const backRouteName = ref('CsSalaryList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csSalary/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csSalary/edit':'/caresys/csSalary/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
