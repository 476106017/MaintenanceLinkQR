...
layout: 'default',
style: {
navigationBarTitleText: 'cs_resident',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="cs_resident"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CsResidentData';
defineOptions({
  name: 'CsResidentList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/caresys/csResident/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/caresys/csResident/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
