        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('系统用户ID（关联sys_user.id）')"
                  name='userId'
                  prop='userId'
                  placeholder="请选择系统用户ID（关联sys_user.id）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['employeeCode']"
                  :label="get4Label('员工编号')"
                  name='employeeCode'
                  prop='employeeCode'
                  placeholder="请选择员工编号"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['phoneNumber']"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['employmentType']"
                  :label="get4Label('雇佣形态（全职/兼职等）')"
                  name='employmentType'
                  prop='employmentType'
                  placeholder="请选择雇佣形态（全职/兼职等）"
                  :rules="[
                  ]"
                  clearable
              />
...
const dataId = ref('')
const backRouteName = ref('CsEmployeeList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csEmployee/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csEmployee/edit':'/caresys/csEmployee/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
