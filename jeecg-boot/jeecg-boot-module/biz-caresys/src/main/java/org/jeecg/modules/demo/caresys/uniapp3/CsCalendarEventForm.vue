...
navigationBarTitleText: 'cs_calendar_event',
},
}
</route>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['eventType']"
                  :label="get4Label('事件类型（访视/培训/检修等）')"
                  name='eventType'
                  prop='eventType'
                  placeholder="请选择事件类型（访视/培训/检修等）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('事件日期')"
                  labelWidth="100px"
                  type="date"
                  name='eventDate'
                  v-model:value="myFormData['eventDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
...
const dataId = ref('')
const backRouteName = ref('CsCalendarEventList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csCalendarEvent/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csCalendarEvent/edit':'/caresys/csCalendarEvent/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
