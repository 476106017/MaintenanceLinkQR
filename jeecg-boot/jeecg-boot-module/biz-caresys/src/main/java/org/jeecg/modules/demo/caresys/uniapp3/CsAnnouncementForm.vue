...
navigationBarTitleText: 'cs_announcement',
},
}
</route>
        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('发布级别（system/company/office）')"
                  name='level'
                  prop='level'
                  placeholder="请选择发布级别（system/company/office）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <DateTime
                  :label="get4Label('可见截止时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='visibleUntil'
                  v-model="myFormData['visibleUntil']"
              ></DateTime>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('CsAnnouncementList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csAnnouncement/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csAnnouncement/edit':'/caresys/csAnnouncement/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
