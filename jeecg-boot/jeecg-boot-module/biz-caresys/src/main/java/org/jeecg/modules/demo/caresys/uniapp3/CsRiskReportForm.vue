...
navigationBarTitleText: 'cs_risk_report',
},
}
</route>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['reportType']"
                  :label="get4Label('报告类型（跌倒等）')"
                  name='reportType'
                  prop='reportType'
                  placeholder="请选择报告类型（跌倒等）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <DateTime
                  :label="get4Label('报告时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='reportTime'
                  v-model="myFormData['reportTime']"
              ></DateTime>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('详情描述')"
                 labelWidth="100px"
                 type="textarea"
                 name='description'
                 prop='description'
...
const dataId = ref('')
const backRouteName = ref('CsRiskReportList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csRiskReport/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csRiskReport/edit':'/caresys/csRiskReport/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
