...
navigationBarTitleText: 'cs_care_record',
},
}
</route>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['residentId']"
                  :label="get4Label('关联老人')"
                  name='residentId'
                  prop='residentId'
                  placeholder="请选择关联老人"
                  :rules="[
                                  { required: true, message: '请输入关联老人!'},
                  ]"
                  clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('执行员工')"
                  name='employeeId'
                  prop='employeeId'
                  placeholder="请选择执行员工"
                  :rules="[
                                  { required: true, message: '请输入执行员工!'},
                  ]"
                  clearable
              />
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <DateTime
                  :label="get4Label('服务时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='recordTime'
                  v-model="myFormData['recordTime']"
              ></DateTime>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('服务内容')"
                 labelWidth="100px"
                 type="textarea"
                 name='serviceContent'
                 prop='serviceContent'
                 clearable
                 :maxlength="300"
                v-model="myFormData['serviceContent']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
...
const dataId = ref('')
const backRouteName = ref('CsCareRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csCareRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csCareRecord/edit':'/caresys/csCareRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
