...
<template>
  <PageLayout navTitle="cs_elder_circle_post"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CsElderCirclePostData';
defineOptions({
  name: 'CsElderCirclePostList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/caresys/csElderCirclePost/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/caresys/csElderCirclePost/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
