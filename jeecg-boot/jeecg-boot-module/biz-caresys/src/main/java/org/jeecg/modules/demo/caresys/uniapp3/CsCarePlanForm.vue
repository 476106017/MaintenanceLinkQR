...
navigationBarTitleText: 'cs_care_plan',
},
}
</route>
    ...
    <scroll-view class="scrollArea" scroll-y>
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('计划开始日期')"
                  labelWidth="100px"
                  type="date"
                  name='planDate'
                  v-model:value="myFormData['planDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['repeatType']"
                  :label="get4Label('重复类型（日/周）')"
                  name='repeatType'
                  prop='repeatType'
                  placeholder="请选择重复类型（日/周）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('服务内容（JSON）')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['serviceItems']"
              ></wd-textarea>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('CsCarePlanList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/caresys/csCarePlan/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/caresys/csCarePlan/edit':'/caresys/csCarePlan/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
