      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['itemName']"
                  :label="get4Label('物品名称')"
                  name='itemName'
                  prop='itemName'
                  placeholder="请选择物品名称"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-textarea
                :label="get4Label('物品描述')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['itemDesc']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('图片URL')"
                 labelWidth="100px"
                 type="textarea"
                 name='imageUrls'
                 prop='imageUrls'
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('物品所在地')"
                  name='location'
                  prop='location'
                  placeholder="请选择物品所在地"
                  :rules="[
                  ]"
                  clearable
              />
...
const dataId = ref('')
const backRouteName = ref('BItemList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/cashless/bItem/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/cashless/bItem/edit':'/cashless/bItem/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
