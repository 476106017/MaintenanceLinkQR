...
<template>
  <PageLayout navTitle="物品需求"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './BItemDemandData';
defineOptions({
  name: 'BItemDemandList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/cashless/bItemDemand/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/cashless/bItemDemand/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
