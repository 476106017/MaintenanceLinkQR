...
<template>
  <PageLayout navTitle="物品评价"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './BReviewData';
defineOptions({
  name: 'BReviewList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/cashless/bReview/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/cashless/bReview/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
