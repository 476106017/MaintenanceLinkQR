      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['tradeId']"
                  :label="get4Label('关联交易ID')"
                  name='tradeId'
                  prop='tradeId'
                  placeholder="请选择关联交易ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['reviewerId']"
                  :label="get4Label('评价人ID')"
                  name='reviewerId'
                  prop='reviewerId'
                  placeholder="请选择评价人ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['targetUserId']"
                  :label="get4Label('被评价人ID')"
                  name='targetUserId'
                  prop='targetUserId'
                  placeholder="请选择被评价人ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['rating']"
                   :label="get4Label('评分 1-5')"
                   name='rating'
                   prop='rating'
                   placeholder="请选择评分 1-5"
                   inputMode="numeric"
                   :rules="[
...
const dataId = ref('')
const backRouteName = ref('BReviewList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/cashless/bReview/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/cashless/bReview/edit':'/cashless/bReview/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
