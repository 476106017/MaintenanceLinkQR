      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['itemId']"
                  :label="get4Label('收藏的物品')"
                  name='itemId'
                  prop='itemId'
                  placeholder="请选择收藏的物品"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('BFavoriteList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/cashless/bFavorite/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/cashless/bFavorite/edit':'/cashless/bFavorite/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
