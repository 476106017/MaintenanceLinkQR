      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['fromItemId']"
                  :label="get4Label('发起人提供物品')"
                  name='fromItemId'
                  prop='fromItemId'
                  placeholder="请选择发起人提供物品"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['toItemId']"
                  :label="get4Label('响应人提供物品')"
                  name='toItemId'
                  prop='toItemId'
                  placeholder="请选择响应人提供物品"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('留言')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['message']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <DateTime
                  :label="get4Label('达成交易时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='dealTime'
                  v-model="myFormData['dealTime']"
              ></DateTime>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('BTradeList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/cashless/bTrade/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/cashless/bTrade/edit':'/cashless/bTrade/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
