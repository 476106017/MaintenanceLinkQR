...
<template>
  <PageLayout navTitle="交易记录"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './BTradeData';
defineOptions({
  name: 'BTradeList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/cashless/bTrade/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/cashless/bTrade/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
