        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('需求发起人')"
                  name='userId'
                  prop='userId'
                  placeholder="请选择需求发起人"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['expectedCategory']"
                  :label="get4Label('分类')"
                  name='expectedCategory'
                  prop='expectedCategory'
                  placeholder="请选择分类"
                  :rules="[
                  ]"
                  clearable
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['expectedName']"
                  :label="get4Label('物品名称关键词')"
                  name='expectedName'
                  prop='expectedName'
                  placeholder="请选择物品名称关键词"
                  :rules="[
                  ]"
                  clearable
              />
...
const dataId = ref('')
const backRouteName = ref('BItemDemandList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/cashless/bItemDemand/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/cashless/bItemDemand/edit':'/cashless/bItemDemand/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
