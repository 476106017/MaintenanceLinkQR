...
<template>
  <PageLayout navTitle="收藏"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './BFavoriteData';
defineOptions({
  name: 'BFavoriteList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/cashless/bFavorite/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/cashless/bFavorite/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
