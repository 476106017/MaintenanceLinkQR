...
<template>
  <PageLayout navTitle="闲置物品"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/cashless/bItem/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/cashless/bItem/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
